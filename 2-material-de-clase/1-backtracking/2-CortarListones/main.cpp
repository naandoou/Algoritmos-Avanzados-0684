...
int generacortes(int x,int y,int n,int m,int ncorte,
    int tablero[N][M],int solu[N][M],int mov[8][2],int dir[4][2]) {
    int nx,ny;
        ...
        if (validapos(nx,ny,tablero,n,m)) {
            ...
            for (int j=0;j<MAX_DIR;j++) {
                ...
                if (validarecorrido(nx,ny,nfinx,nfiny,n,m,tablero)) {
                    marca(nx,ny,nfinx,nfiny,n,m,ncorte,tablero);
                    if (maxcorte<ncorte) {
                        maxcorte=ncorte;
                        for (int p=0;p<n;p++)
                            for (int q=0;q<m;q++)
                                solu[p][q]=tablero[p][q];
                        cout << maxcorte << endl;
                    }
                    if (generacortes(nfinx,nfiny,n,m,ncorte+1,tablero,solu,mov,dir))
                        return 1;
...
int main() {
    ...
    int mov[8][2],dir[4][2],tablero[N][M],solu[N][M];
    ...
    generacortes(x,y,n,m,1,tablero,solu,mov,dir);
    imprime(solu,n,m);
