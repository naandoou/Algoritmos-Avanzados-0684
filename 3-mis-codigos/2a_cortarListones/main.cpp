...
int validarecorrido(int x, int y, int xfin, int yfin,int tablero[N][M], int n, int m) {
...
void marca(int xini, int yini, int xfin, int yfin, int nmov, int tablero[N][M]) {
    ...
    if (xini < xfin) for (int i = xini; i <= xfin; i++) tablero[i][yini] = nmov;
    if (yini > yfin) for (int i = xini; i >= xfin; i--) tablero[xini][i] = nmov;
}

int generacortes(int x, int y, int n, int m, int nmov, int tablero[N][M], int mov[8][2], int dir[4][2]) {
    int ninix, niniy;
        ...
        if (validaposicion(ninix, niniy, tablero, n, m)) {
            ...
            for (int j = 0; j < MAX_DIR; j++) {
                ...
                if (validarecorrido(ninix, niniy, nfinx, nfiny, tablero, n, m)) {
                    ...
                    if (generacortes(nfinx, nfiny, n, m, nmov + 1, tablero, mov, dir)) return 1; // cuando es correcto, devuelve 1
                    marca(ninix, niniy, nfinx, nfiny, nmov, tablero);
                }
            }
        }
...
int main() {
    ...
    // backtracking cortes
    int x = -1, y = -1;
    generacortes(X, Y, N, M, 1, tablero, mov, dir); // funcion recursiva
    imprime(tablero);
